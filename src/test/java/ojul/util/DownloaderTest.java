...
public class DownloaderTest implements ResourcePath {
	...
	public void tearDown() throws Exception {
		...
		try {
			...
			new File(ResourcePath.JAVA_HOME).delete();
			new File(ResourcePath.TEMP_HOME).delete();
			new File(ResourcePath.OJUL_HOME).delete();
	...
	public void testcheckDirectories_1() {
		...
		Assert.assertTrue(new File(ResourcePath.OJUL_HOME).exists());
		Assert.assertTrue(new File(ResourcePath.JAVA_HOME).exists());
		Assert.assertTrue(new File(ResourcePath.TEMP_HOME).exists());
	}
	...
	public void testDownloading() {
		...
		System.out.println(
		        "Download is completed : " + this.downloader.isDownloadComplete());
	}
}
