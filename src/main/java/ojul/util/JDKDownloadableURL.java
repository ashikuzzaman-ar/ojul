...
/**
 * This class will find out version specific url for latest jdk. This class is needed
 * because oracle does not allow to access direct download links and such links changes
 * dynamically for every new jdk update.
 *
 * @author ashik
 *
 */
public class JDKDownloadableURL implements DownloadableURL {
	...
	private final List<JavaDetails> javaDetails;

	private final String jdkURL;
	...
	public JDKDownloadableURL() {
		...
		this.javaDetails = new LinkedList<>();
	}
	...
	public String getCurrentJDKVersion() {
		...
		return this.currentJDKVersion != null ? this.currentJDKVersion
	...
	private String getDownloadPageURL() {
		...
		try {
			...
			final Document document = Jsoup.connect(this.jdkURL).get();
			// Parsing div element having id 'Wrapper_FixedWidth_Centercontent'
			Element element = document
			        .getElementById("Wrapper_FixedWidth_Centercontent");
	...
	private String getLatestJDKVersion() {
		...
		try {
			...
			final Document document = Jsoup.connect(this.getDownloadURL()).get();
			...
			final Element element = document.getElementsByTag("script").get(9);
			...
			for (final String item : element.toString().split("\n")) {

				// Traversing through lines to find out those lines starting with
				// "downloads"
				...
				if (item.startsWith("downloads") && item.contains("Linux")) {
					...
					final JavaDetails details = mapper.readValue(item.split("=")[1],
					        JavaDetails.class);
					// Splitting using "']['" and taking 3rd element cleaning "']" from
					// the
					// last of the line
					details.setFileName(item.split("=")[0].split("'\\]\\['")[2]
					        .replaceAll("'\\]", ""));
			...
			this.currentJDKVersion = this.javaDetails.get(0).getFileName().split("_")[0]
			        .split("-")[1];
